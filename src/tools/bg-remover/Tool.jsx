...
export default function BgRemoverTool() {
  ...
  const removeBackground = async () => {
    ...
    try {
      // 1. Import dependencies dynamically for client-side execution
      const { remove, newSession, rembgConfig } = await import("@bunnio/rembg-web");
      const ort = await import("onnxruntime-web");
      
      // 2. Fix for Vercel: Point WASM engine to CDN to avoid 404 local path errors
      ort.env.wasm.wasmPaths = "https://cdn.jsdelivr.net/npm/onnxruntime-web@1.18.0/dist/";
      
      setProgress(30);

      // 3. Configure the Silueta model (High quality, smaller size for faster web loading)
      ...
      // 4. Initialize and run the session
    ...
    <div className="space-y-8">
      {!originalImage && (
        ...
        >
          ...
          <p className="text-muted text-sm mt-2">or click to browse — PNG, JPG, WEBP supported</p>
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="chrome-card">
            <div className="chrome-bar">
              ...
              <div className="chrome-dot" />
              <span className="text-xs font-mono text-muted ml-2">original.jpg</span>
            </div>
            <div className="p-4 bg-[url('data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2220%22%20height%3D%2220%22%3E%3Crect%20width%3D%2210%22%20height%3D%2210%22%20fill%3D%22%23ddd%22%2F%3E%3Crect%20x%3D%2210%22%20y%3D%2210%22%20width%3D%2210%22%20height%3D%2210%22%20fill%3D%22%23ddd%22%2F%3E%3C%2Fsvg%3E')]">
              <img src={originalImage.url} alt="Original" className="w-full h-52 sm:h-64 object-contain" />
            </div>
          </div>
          ...
          <div className="chrome-card">
            <div className="chrome-bar">
              <div className="chrome-dot" />
              <div className="chrome-dot" />
              <span className="text-xs font-mono text-muted ml-2">removed-bg.png</span>
            </div>
            <div className="p-4 h-[calc(100%-41px)] flex items-center justify-center bg-[url('data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2220%22%20height%3D%2220%22%3E%3Crect%20width%3D%2210%22%20height%3D%2210%22%20fill%3D%22%23ccc%22%2F%3E%3Crect%20x%3D%2210%22%20y%3D%2210%22%20width%3D%2210%22%20height%3D%2210%22%20fill%3D%22%23ccc%22%2F%3E%3C%2Fsvg%3E')]">
              {resultImage ? (
                <img src={resultImage} alt="Result" className="w-full h-52 sm:h-64 object-contain" />
              ) : (
      ...
      {error && <p className="text-accent text-sm font-mono border border-accent/30 bg-accent/5 rounded-lg px-4 py-3">✕ {error}</p>}
      ...
      {originalImage && (
        <div className="flex gap-3 flex-wrap">
          {!resultImage ? (
            ...
            <button onClick={handleDownload} className="w-full sm:w-auto bg-accent text-white font-display font-bold px-8 py-3 rounded-lg hover:bg-ink transition-colors">
              ↓ Download PNG
            </button>
          )}
          <button
      ...
      <p className="text-xs text-muted font-mono">✦ Runs 100% in your browser. Your images never leave your device.</p>
    </div>
  );
}